...
const Comments = ({style,path,placeholder,limit,justComments,commentStyle}) => {
    ...
    return (
        <div style={{flex:1,...style}} onLayout={(e)=>setWidth(e.nativeEvent.layout.width)}>
            {!justComments && <><Row style={{flex:1}}>
                <div style={{flexGrow:1}}>
                    {!name && <input style={{marginRight:5,marginBottom:5,padding:10,backgroundColor:'#ffffff'}} value={author} onChangeText={setAuthor} placeholder="Név"/>}
                    <input style={{flex:1,marginRight:5,marginBottom:0,padding:10,backgroundColor:'#ffffff'}} multiline rows={2} value={text} 
                    onChangeText={setText} 
                    placeholder={(placeholder) ? placeholder : 'Kommented'}/>
                </div>
                <NewButton title={width > 300 ? 'Küldés' : <ArrowRedoOutline size={30} color='black'/>} 
            ...
            {!!list?.length &&<div style={{flexWrap:'wrap',flexDirection:'row',margin:10,marginRight:15}}>
                {list.map((comment,ind)=>{

                    return (
                        <div key={'comment'+ind} style={{background:'white',padding:5,margin:5,maxWidth:'100%',...commentStyle}}>
                            <div onClick={()=>{
                                if (comment?.uid)
