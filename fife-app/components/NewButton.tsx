...
export function NewButton({title,onPress,disabled,selected,style,textStyle,floating,icon,color = "#fdcf99",info,loading=false}) {
    const ref = useRef();
    ...
    return (<>
      ...
      style={{margin:0,padding:0,width:'unset',height:'unset',alignItems:'unset',boxShadow:'unset',border:'none',...style}}>
        <div onPressOut={()=>setBgColor(color)} onClick={()=>{setBgColor(color);handleFocus()}} 
              ...
              {!loading ? 
                <span style={{ fontWeight: 'bold', color: isBright(bgColor) , fontSize:16,...textStyle}}>{title}</span>
              : <span>loading</span> }
        </div>
      </div>
      {info && rect && isHovered && <span 
