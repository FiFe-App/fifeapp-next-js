...
const ctx = ({ children }) => {
            ...
            await Notification.requestPermission()
                .then((permission) => {
                    ...
                    if (permission !== 'granted') {
                        ...
                        getToken(messaging, {
                            vapidKey:
                                'BInTt__OonGUhBNBdQA57cu-VRHBm6N7vcsJBe_Q3o1Ei_2UgPSfM0ZzxyXsxohdrV_qooAywYzRilIv5OJ6VQE',
    ...
    const appCheck = () => {
        ...
        try {
            const appCheckObj = initializeAppCheck(getApp(), {
    ...
    const forgotPassword = async (email) => {
        ...
        if (!email || email == '')
            return 'Email nélkül nem tudjuk visszaállítani a jelszavad'
    ...
    const logout = () => {
        ...
        if (msg) {
            deleteToken(msg)
                .then((e) => console.log('token deleted?', e))
                ...
                await signInWithEmailAndPassword(
                        ...
                        if (firstLogin) {
                            ...
                            try {
                                ...
                                await set(
                                    ref(
                                        getDatabase(),
                                        `users/${user.uid}/settings/homeFilter`
                                    ),
                                    firstLogin.interest
                                ).then(() => {
                                    console.log('RTDB updated')
                    ...
                    .catch((error) => {
                        ...
                        if (
                            ...
                            response = {
                                error: 'Bakfitty! Nem jó az email cím, amit megadtál!',
                            }
                        else if (errorCode == 'auth/internal-error')
                            response = {
                                error: 'Azáldóját! A szerveren hiba történt, próbáld újra!',
                            }
                        else if (errorCode == 'auth/wrong-password')
                            response = {
                                error: 'Azt a hét meg a nyolcát! Lehet elírtad a jelszavad.',
                            }
                        else if (errorCode == 'auth/too-many-requests')
                            response = {
                                error: 'Ó te jó ég! Túl sokszor próbáltál bejelentkezni, próbálkozz később!',
                            }
                        else
                            response = {
                                error:
        ...
        const userCredential = await createUserWithEmailAndPassword(
            ...
            .catch((error) => {
                ...
                if (errorCode == 'auth/invalid-email')
                    ...
                    response = {
                        error: 'A jelszavad nem elég bonyi\nlegyen legalább 6 karakter',
                    }
                else if (errorCode == 'auth/wrong-password')
