...
function urlify(text) {
    var urlRegex =
        /(([a-z]+:\/\/)?(([a-z0-9\-]+\.)+([a-z]{2}|aero|arpa|biz|com|coop|edu|gov|info|int|jobs|mil|museum|name|nato|hu|net|org|pro|travel|local|internal))(:[0-9]{1,5})?(\/[a-z0-9_\-\.~]+)*(\/([a-z0-9_\-\.]*)(\?[a-z0-9+_\-\.%=&amp;]*)?)?(#[a-zA-Z0-9!$&'()*+.=-_~:@/?]*)?)(\s+|$)/gi
...
function ToHigh(vowel) {
    ...
    if (i >= 0) return high.slice(i, i + 1)
    else return vowel
}
...
function toLong(vowel) {
    ...
    if (i >= 0) return long.slice(i, i + 1)
    else return vowel
}
...
export const toldalek = (str, toldalek) => {
    ...
    if (isVowel(toldE)) {
        ...
        if (isVowel(str.slice(-1)))
            return str.slice(0, -1) + toLong(str.slice(-1)) + toldalek
        else if (
            str.slice(-1) == str.slice(-2, -1) &&
            str.slice(-2, -1) == toldalek.slice(0, 1)
        )
            return str + toldalek.slice(1)
...
function getSynonims(key) {
    const request = new Request(
        'https://cors-anywhere.herokuapp.com/https://api.poet.hu/szinonima.php?f=akoskristof&j=e7a5cefcde99071ab9001c91f3d892e0&s=' +
            key
    )
...
export async function search(key, array, withSynonims) {
    ...
    let synonims = []
    if (withSynonims) synonims = await getSynonims(key)
